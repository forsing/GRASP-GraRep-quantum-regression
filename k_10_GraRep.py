...
# Varijanta: GraRep embedding (potencije P^k, SVD po redosledu, konkatenacija)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
GraRep = više redosleda (P^1, P^2, ...), SVD za svaki, konkatenacija embeddinga
"""
...
GRAREP_ORDERS = [1, 2] # redosledi (P^1, P^2)
GRAREP_DIMS = [2, 1]   # broj SVD komponenti (2, 1)
...
def compute_grarep_embeddings(M, k=EMBED_DIM):
    ...
    parts = []
    for ord_k, d in zip(GRAREP_ORDERS, GRAREP_DIMS):
        Pk = np.linalg.matrix_power(P, ord_k)
        U, S, Vh = np.linalg.svd(Pk, full_matrices=False)
        d_use = min(d, len(S))
        part = U[:, :d_use] * np.sqrt(np.maximum(S[:d_use], 1e-12))
        parts.append(part)
    emb = np.hstack(parts)
    if emb.shape[1] > k:
        emb = emb[:, :k]
...
def main():
    ...
    emb = compute_grarep_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (GraRep + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    """
    Predikcija (GraRep + kvantna regresija, deterministički, strukturno):
    (8, 11, 23, 26, 28, 33, 34)
    """
...
P = red-normalizovana susednost.

Redosledi 1 i 2: 
za redosled 1 uzimaju se 2 komponente (SVD od P^1), 
za redosled 2 jedna (SVD od P^2); 
rezultati se nadovezuju u (39, 3).

Konstante: GRAREP_ORDERS = [1, 2], GRAREP_DIMS = [2, 1]. 
